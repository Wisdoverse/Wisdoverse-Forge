...
export const zh: TranslationKeys = {
  ...
  skills: {
    detail: {
      ...
      allAgentsTooltip: '这条保存的指引不需要指定文件编辑工具。',
      containerCliTooltip: '文件编辑工具：{{tool}}',
      unknownToolTooltip: '打开设置，检查文件编辑工具，然后再使用这条保存的指引。',
      nextStepHeading: '下一步做什么',
      nextStepReady: '创建任务时可以使用这条保存的指引，也可以让匹配词在类似任务中提示它。',
      nextStepNeedsInstall: '请让所有者或管理员先完成设置，然后再在任务中使用这条保存的指引。',
      sourceLabel: '保存位置',
      authorLabel: '更新人',
      availabilityLabel: '谁可以使用',
      descriptionHeading: '它能帮什么',
      noDescription: '使用这条保存的指引前，请先查看下面的可复用指引。',
      triggerHeading: '什么时候有帮助',
      triggerHelper: '任务里出现类似这些词时，可以推荐这条保存的指引。',
      detailsHeading: '可复用指引',
      detailsHelper: '使用这条保存的指引前，请先查看这些可复用步骤。',
      noContent: '还没有保存可复用步骤。请先补充智能体要遵循的步骤，再使用这条保存的指引。',
      unknownAuthor: '请重新打开保存的指引，查看谁负责更新它',
      unknownSource: '保存的指引',
