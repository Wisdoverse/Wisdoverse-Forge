...
function createTeamErrorMessage(error: unknown): string {
  ...
  if (code === 404) {
    return 'Open Settings, then Teams again, then create this team. The team space may have changed.'
  }
  ...
  return (
    ...
    >
      ...
      <div className="mb-3">
        ...
        {name.trim() && (
          <p className="mt-1 text-ui-caption text-secondary-light dark:text-secondary-dark">
            Team menu link preview: {slugifyName(name)}. Forge creates it automatically from the
            team name. You do not need to type it.
          </p>
        )}
      </div>
      {visibleError && (
