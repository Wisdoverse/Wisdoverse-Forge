...
function agentStatusLabel(status: string): string {
  switch (status.trim().toLowerCase()) {
    ...
    default:
      return status.trim() ? 'Check agent status' : 'Check agents again to confirm status'
  }
}
...
function agentOwnerLabel(agent: AdminAgent): string {
  return readableValue(
    agent.ownerUsername,
    readableValue(agent.ownerEmail, 'Check agents again to load owner')
  )
}
...
function agentStatusDot(status: string): string {
  switch (status.trim().toLowerCase()) {
    case 'working':
      return 'bg-[#1d1d1f] dark:bg-white'
    case 'idle':
      return 'bg-[#7a7a7a]'
    default:
      return 'bg-[#d2d2d7]'
...
function AgentsGuide({
  ...
  return (
    <section data-testid="admin-agents-guide" className={cn(uiStyles.cardPadded, 'mb-4')}>
      <div className="mb-3">
                ...
                >
                  ...
                  <td className={uiStyles.tableCell}>
                    <span className="inline-flex items-center gap-1.5 text-ui-body text-secondary-light dark:text-secondary-dark">
                      <span
                        className={cn('h-1.5 w-1.5 rounded-full', agentStatusDot(agent.status))}
                      />
                  ...
                  >
                    {readableValue(agent.projectName, 'Check agents again to load project')}
                  </td>
                  <td
                    className={cn(
                      uiStyles.tableCell,
