...
export function runHistoryErrorMessage(err: unknown): string {
  const base = 'Run attempts could not be loaded.'
  ...
  if (code === 403 || text.includes('forbidden')) {
    return `${base} Ask an owner or admin to give you access to this task.`
  }
  if (code === 404) {
    return `${base} Refresh the page; this task may have changed or been removed.`
  }
  if (code === 429 || text.includes('rate limit') || text.includes('too many')) {
    return `${base} Too many task history requests are happening right now. Wait a minute, then try again.`
  }
