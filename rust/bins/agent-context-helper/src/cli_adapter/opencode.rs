...
pub fn apply_opencode_adapter(envelope: &ContextEnvelope, home: &Path) -> Result<ContextAdapterReport> {
    let opencode_dir = home.join(".config/opencode");
    fs::create_dir_all(&opencode_dir).with_context(|| format!("create {}", opencode_dir.display()))?;
...
fn render_context_block(envelope: &ContextEnvelope) -> String {
    ...
    out.push_str("These instructions are generated for opencode from the approved AgentForge context envelope.\n");
    out.push_str("opencode loads this file from the global ~/.config/opencode/AGENTS.md rules layer.\n\n");
