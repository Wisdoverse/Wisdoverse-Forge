...
impl TaskImageMaterializer {
    ...
    pub async fn materialize_for_dispatch(
        ...
        if image_ids.len() > MAX_TASK_IMAGES {
            return Err(instruction_image::too_many_task_images(MAX_TASK_IMAGES));
        }
        ...
        if agent.runtime_kind != RuntimeKind::Container {
            return Err(instruction_image::image_tasks_container_only());
        }
        // Capability gate: only a vision-capable container CLI tool can consume images.
        let cli_tool = agent.cli_tool.as_deref().ok_or_else(instruction_image::image_task_requires_container_cli)?;
        let kind = CliToolKind::parse_legacy(cli_tool).map_err(instruction_image::invalid_cli_tool)?;
        if !RuntimeCapability::for_cli_tool(kind, RuntimeKind::Container).supports_image_input {
            return Err(instruction_image::cli_tool_does_not_support_images(cli_tool));
        }
