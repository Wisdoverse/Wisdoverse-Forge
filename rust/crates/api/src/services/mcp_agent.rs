...
pub trait McpAgentTools: Send + Sync {
    ...
    async fn send_prompt(&self, org_id: Uuid, agent_id: Uuid, prompt: &str) -> AppResult<()>;
    async fn destroy_session(&self, org_id: Uuid, agent_id: Uuid) -> AppResult<()>;
    async fn session_status(&self, org_id: Uuid, agent_id: Uuid) -> AppResult<SessionStatus>;
}
    ...
    R: McpAgentRuntime,
{
    ...
    /// authoritative org. A mismatch returns the same not-found error as a missing
    /// agent so cross-org existence is not leaked.
    async fn require_agent_in_org(&self, org_id: Uuid, agent_id: Uuid) -> AppResult<()> {
    ...
    pub async fn send_prompt(&self, org_id: Uuid, agent_id: Uuid, prompt: &str) -> AppResult<()> {
        self.require_agent_in_org(org_id, agent_id).await?;
        let prompt = McpAgentPrompt::parse(prompt)?;
        self.runtime.send_prompt(agent_id, prompt.content()).await
    }

    pub async fn destroy_session(&self, org_id: Uuid, agent_id: Uuid) -> AppResult<()> {
        self.require_agent_in_org(org_id, agent_id).await?;
        self.runtime.destroy_agent(agent_id).await?;
        self.store.delete_agent(agent_id).await
    }

    pub async fn session_status(&self, org_id: Uuid, agent_id: Uuid) -> AppResult<SessionStatus> {
        self.require_agent_in_org(org_id, agent_id).await?;
    ...
    R: McpAgentRuntime,
{
    ...
    async fn send_prompt(&self, org_id: Uuid, agent_id: Uuid, prompt: &str) -> AppResult<()> {
        McpAgentService::send_prompt(self, org_id, agent_id, prompt).await
    }

    async fn destroy_session(&self, org_id: Uuid, agent_id: Uuid) -> AppResult<()> {
        McpAgentService::destroy_session(self, org_id, agent_id).await
    }

    async fn session_status(&self, org_id: Uuid, agent_id: Uuid) -> AppResult<SessionStatus> {
        McpAgentService::session_status(self, org_id, agent_id).await
    }
}
