...
use crate::services::governance_audit::{
    GovernanceAuditService, QueryParams as GovernanceAuditQueryParams, governance_audit_hmac_key,
    governance_audit_response,
};
...
fn make_service(state: &AppState) -> AppResult<GovernanceAuditService> {
    Ok(GovernanceAuditService::new(
        GovernanceAuditRepository::new(state.pool.clone()),
        AuditRepository::new(state.pool.clone()),
        governance_audit_hmac_key(state.config.is_production(), state.encryption_key)?,
    ))
}
