...
async fn create_image_attachment(
    ...
    // The service resolves the agent's workspace (repository wiring stays out of the
    // route) so the upload is recorded against the agent's own workspace.
    let att = make_service(&state).create_image_upload_for_agent(&auth.scope, agent_id, &filename, bytes).await?;
    Ok((StatusCode::CREATED, Json(attachment_data_response(att))))
}
...
async fn parse_image_upload(mut multipart: Multipart) -> AppResult<(Vec<u8>, String, AgentId)> {
    ...
    let bytes = bytes.ok_or_else(|| AttachmentMultipartPolicy::required_field("file"))?;
    let agent_id = agent_id.ok_or_else(|| AttachmentMultipartPolicy::required_field("agent_id"))?;
