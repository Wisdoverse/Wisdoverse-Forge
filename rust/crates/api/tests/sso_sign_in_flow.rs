            ...
            get(move |headers: axum::http::HeaderMap| {
                ...
                async move {
                    ...
                    let body = if auth == "Bearer idp-token-123" {
                        let mut value = json!({"email":"sso-user@example.com","name":"SSO User"});
...
async fn callback_exchanges_and_provisions_then_exchanges_signs_in(pool: PgPool) {
    ...
    let user =
        user_service.ensure_sso_user("sso-user@example.com", Some("SSO User")).await.expect("reprovision idempotent");
    ...
    let result = service.exchange(&code, &user_service).await.expect("exchange");
    assert_eq!(result.user.email, "sso-user@example.com");
    let claims = service_verify(&service, &result.access_token);
    ...
    assert_eq!(idp.token_hits.load(std::sync::atomic::Ordering::SeqCst), 1);
    assert_eq!(idp.userinfo_hits.load(std::sync::atomic::Ordering::SeqCst), 1);
}
...
fn service_verify(_service: &SsoService, token: &str) -> agentforge_auth::claims::Claims {
    // Re-verify with a JwtManager over the same secret: the service holds its
    // own manager, so the test signs nothing — it only reads back the claims.
