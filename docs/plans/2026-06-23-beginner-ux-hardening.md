...
| Agents list, detail, and lifecycle actions                   | `src/app/routes/agents.tsx`, `src/app/entities/agent/model/agents.store.ts`, `src/app/features/agents/*`, `src/app/widgets/agent-detail/AgentDetailView.tsx`                                           | `agents.store.test.ts`, `AgentListView.test.tsx`, `AgentDetailView.test.tsx`, `AgentControlPanel.test.tsx`, `CreateAgentModal.test.tsx`, `AgentPluginsTab.test.tsx`, `AgentTasksTab.test.tsx`                                                                                                                               | DONE   | Phase 3 reviewed create/connect/start/restart/delete as a user journey; continue only if a later audit finds a rendered lifecycle gap.              |
| Tasks board, list, timeline, and visual map                  | `src/app/routes/tasks.tsx`, `src/app/features/board/*`, `src/app/features/list/ListView.tsx`, `src/app/widgets/views/TimelineView.tsx`, `src/app/widgets/views/Workshop3DView.tsx`                     | `BoardView.test.tsx`, `TaskFormModal.test.tsx`, `TaskCard.test.tsx`, `KanbanColumn.test.tsx`, `ListView.test.tsx`, `TimelineView.test.tsx`, `Workshop3DView.test.tsx`, `BoardToolbar.test.tsx`                                                                                                                              | DONE   | Phase 3 verified empty, loading, create, assign, blocked, failed, retry, timeline, and visual map states. Continue only if final audit finds a gap. |
| Task detail, history, result review, and save-as-instruction | `src/app/features/detail/*`, `src/app/features/detail/model/*`, `src/app/widgets/agent-detail/AgentDetailView.tsx`                                                                                     | `TaskDetailPanel.test.tsx`, `HistoryTab.test.tsx`, `TaskMetadata.test.tsx`, `SkillDraftModal.test.tsx`, `reviewSnapshotErrorMessage.test.ts`, `taskDetailErrorMessages.test.ts`, `skillDraftErrorMessage.test.ts`                                                                                                           | DONE   | Phase 3 verified result review, retry/allow recovery, and saved-instruction next steps after useful work completes.                                 |
...
| Inbox and activity feed                                      | `src/app/routes/inbox.tsx`, `src/app/features/inbox/*`, `src/app/features/feed/*`, `src/app/shared/model/feed.store.ts`                                                                                | `InboxView.test.tsx`, `feed.store.test.ts`, `AgentStatusBar.test.tsx`                                                                                                                                                                                                                                                       | DONE   | Phase 3 verified needs-action ordering, empty states, account access actions, read-state recovery, agent status copy, and feed retention guards.    |
...
| Platform CLI and operator docs                               | `rust/bins/cli`, `rust/crates/cli`, `docs/guides/cli-platform-support.md`, setup/deployment runbooks                                                                                                   | `agentforge` root help test, `agentforge agents enroll-local` help test, Host CLI join-script template test, `npm run beginner:ux:copy`, docs review                                                                                                                                                                        | DONE   | Phase 4 verified the Platform CLI, sidecar help, Host CLI runbook, README, and Getting Started entry points for beginner-safe cross-platform setup. |
...
- Agents: create/connect/start/restart/delete already show prerequisites,
  success states, confirmation for risky actions, and recoverable error copy.
  Current coverage is in `tests/unit/app/AgentListView.test.tsx`,
  `tests/unit/app/CreateAgentModal.test.tsx`,
  `tests/unit/app/AgentControlPanel.test.tsx`, and
  `tests/unit/app/AgentDetailView.test.tsx`. No confirmed code gap was found in
  this Phase 3 review.
...
- Tasks: board loading, missing project/task waiting place, lifecycle columns,
  assignment readiness, search/filter empty states, failed and blocked cards,
  title-only task drafts, list view next steps, timeline start guidance, and
  visual map first-agent guidance are covered by
  `tests/unit/app/BoardView.test.tsx`,
  `tests/unit/app/TaskFormModal.test.tsx`,
  `tests/unit/app/TaskCard.test.tsx`,
  `tests/unit/app/KanbanColumn.test.tsx`,
  `tests/unit/app/ListView.test.tsx`,
  `tests/unit/app/TimelineView.test.tsx`,
  `tests/unit/app/Workshop3DView.test.tsx`, and
  `tests/unit/app/BoardToolbar.test.tsx`.
- Task detail: result review, retry/allow recovery, task updates, and
  save-as-instruction next steps are covered by
  `tests/unit/app/TaskDetailPanel.test.tsx`,
  `tests/unit/app/HistoryTab.test.tsx`, and
  `tests/unit/app/SkillDraftModal.test.tsx`. Retry/allow failure copy now also
  uses `role="alert"` with `aria-live="polite"`.
- Saved instructions: create, required fields, denied saves, load failures,
  empty states, search, filters, detail cards, and source-name hiding are
  covered by `tests/unit/app/SkillsView.test.tsx`,
  `tests/unit/app/SkillCard.test.tsx`,
  `tests/unit/app/SkillDetailModal.test.tsx`, and
  `tests/unit/app/skills.store.test.ts`. A saved instruction now confirms
  success in the `aria-live` toolbar and tells the user to open it to check or
  reuse it on a task.
- Settings providers: AI service loading, empty setup, search/filter empty
  states, setup prerequisites, save errors, connection checks, enable/disable,
  and delete labels are covered by `tests/unit/app/ProvidersSection.test.tsx`.
  A saved AI service now confirms success in an `aria-live` status and tells
  the user to choose Check connection before agents use that service.
- Settings runtime and code access: work setup prerequisites, Codex/work tool
  sign-in recovery, agent online checks, unknown setup values, HTTPS and SSH
  access setup, one-time outside tool access keys, delete confirmations, and raw
  error hiding are covered by `tests/unit/app/RuntimeSection.test.tsx`,
  `tests/unit/app/GitCredentialsSection.test.tsx`,
  `tests/unit/app/SshKeysSection.test.tsx`, and
  `tests/unit/app/KeysSection.test.tsx`.
- Settings team, project, and resources: team empty states, team creation
  success, team/project delete confirmations, edit validation, permission
  recovery, setup reset, resource loading, resource empty states, and agent-size
  retry guidance are covered by `tests/unit/app/TeamsSection.test.tsx`,
  `tests/unit/app/WorkspaceRows.test.tsx`,
  `tests/unit/app/ResourcesSection.test.tsx`,
  `tests/unit/app/workspaceSettingsErrorMessage.test.ts`, and
  `tests/unit/app/AccountSection.test.tsx`.
- Inbox and activity feed: no-update empty states, saved-update loading,
  failed/blocked raw-detail hiding, needs-action ordering, account access
  routing, empty filters, read-state recovery, agent status copy, and feed
  retention are covered by `tests/unit/app/InboxView.test.tsx`,
  `tests/unit/app/feed.store.test.ts`, and
  `tests/unit/app/AgentStatusBar.test.tsx`.
...
- `agentforge-sidecar --help` already tells users to copy the join command from
  the Agents page first and explains what success looks like.
- `README.md` and `docs/guides/getting-started.md` both link operators to Host
  CLI Agent Enrollment and CLI Platform Support before advanced implementation
  details.
