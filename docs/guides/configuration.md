...
For a fresh production database, enter `BOOTSTRAP_ADMIN_TOKEN` in the browser's
Deployment setup token field when creating the first administrator. The
development Compose override explicitly skips this token for disposable local
databases; direct/custom server starts fail closed unless they opt in.
...
| `REQUIRE_EXTERNAL_STATE`            | `false`       | No                     | CN-7. Set `true` when running MORE THAN ONE API replica: the OAuth/PKCE state store must then be Redis (shared across replicas). When `true`, Redis becomes a REQUIRED dependency — the server fails fast at startup unless Redis is configured, reachable, AND writable — it probes the actual `SET`/`GETDEL` path at boot, so a read-only or ACL-restricted Redis is rejected, not just a missing `REDIS_URL`. The `/api/health` readiness probe also returns 503 if Redis later goes down, so the replica drops out of rotation instead of serving 500s from the Redis state store. Leave `false` for single-replica deployments (in-memory state, Redis optional) |
...
| `BOOTSTRAP_ADMIN_TOKEN`             | none          | Fresh production       | One-time secret for creating the first platform administrator. Generate at least 32 random characters; later registrations do not require it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `ALLOW_UNPROTECTED_ADMIN_BOOTSTRAP` | `false`       | No                     | Local development/test only. Set by the development Compose override; ignored in other runtime modes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `AUTH_SSO__DEPROVISION`          | `false`                  | No            | Remove mapped-org memberships when the group no longer applies (owners + last membership protected)
...
browser to `SPA_BASE_URL/login?auth_code=…`. The one-time `auth_code` is
signed for 120 seconds and redeemed at `/api/v1/auth/sso/exchange`. Register
...
upgraded to the org `admin` role in their default team space — existing members
benefit immediately, owners are never touched, and nothing is ever demoted.
...
additionally removes that membership when the group no longer applies — owners
are never removed, and the user's last remaining membership is always kept so
sign-in cannot lock out.
...
The `active` field on POST is accepted for SCIM client compatibility but not
yet enforced; protect a directory-removed user with `DELETE`. SCIM errors use
...
| `TASK_IMAGES_CLEANUP_TTL_SECS`  | `86400`                      | No                              | Age (seconds) after a task ends before its materialized instruction images under `/workspace/.task-images/<task-id>/` are reclaimed by the cleanup sweeper. Lower it on high-churn hosts to reclaim disk sooner. Must be a positive integer; a malformed value or `0` fails startup rather than silently using the default |

Instruction images attached to a task are stored in object storage (above) and
also materialized into the assigned container agent's workspace at dispatch. A
background sweeper removes the per-task workspace copies once the task has been
terminal (completed/failed/canceled) for `TASK_IMAGES_CLEANUP_TTL_SECS`; the
object-storage copy follows the normal attachment lifecycle.
...
| `ORCHESTRATOR_LEADER_ELECTION_ENABLED`       | `false`                                    | No                                | Opt-in (CN-6). When `true` and more than one orchestrator replica runs, only the replica holding a Postgres advisory lock runs each singleton reaper tick (dispatch TTL + review escalation); the others skip. Single-replica deployments are unaffected, and the data-plane `SKIP LOCKED` consumers are never gated                                                                                                                                                                                    |
...
| `DEV_ENV_ALLOWED_IMAGE_REGISTRIES` | empty                                      | Comma-separated extra image-reference prefixes a dev-environment may run (e.g. `ghcr.io/myorg/,docker.io/`). Official Docker Hub library images (e.g. `ubuntu:22.04`) and managed `agentforge-agent` images are always allowed; anything else is rejected. Set this to permit your own registry. |
