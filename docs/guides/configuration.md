...
| `REQUIRE_EXTERNAL_STATE`          | `false`       | No                     | CN-7. Set `true` when running MORE THAN ONE API replica: the OAuth/PKCE state store must then be Redis (shared across replicas). When `true` the server fails fast at startup unless `REDIS_URL` is set, instead of silently using the per-process in-memory store (which splits the authorize/callback steps across replicas). Leave `false` for single-replica deployments                                                            |
