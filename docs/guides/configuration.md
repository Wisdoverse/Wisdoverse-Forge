...
| `REQUIRE_EXTERNAL_STATE`          | `false`       | No                     | CN-7. Set `true` when running MORE THAN ONE API replica: the OAuth/PKCE state store must then be Redis (shared across replicas). When `true`, Redis becomes a REQUIRED dependency — the server fails fast at startup unless Redis is configured AND reachable (not just `REDIS_URL` present), and the `/api/health` readiness probe returns 503 if Redis later goes down, so the replica drops out of rotation instead of serving 500s from the Redis state store. Leave `false` for single-replica deployments (in-memory state, Redis optional)                       |
