...
| SSO role mapping    | With `AUTH_SSO__ROLE_CLAIM`/`AUTH_SSO__ADMIN_GROUPS`, each sign-in maps the provider's groups onto the org role: members in an admin group are upgraded to `admin` in their default team space; owners are never touched and nothing is demoted.                                                                                                                |
...
| SSO org provisioning| With `AUTH_SSO__ORG_GROUP_MAP`, sign-ins add the user to mapped orgs when the provider group matches (member, or admin with an admin group); `AUTH_SSO__DEPROVISION` removes it when the group leaves — owners and the user's last membership are always protected.                                                                                                                        |
...
| Offline bundles | `scripts/offline-bundle.sh --full-stack` packages the Forge images plus pinned platform services into one verified tar; `scripts/load-offline-bundle.sh` checks `SHA256SUMS` and loads it on an air-gapped host (see `docs/guides/offline-install.md`). |
...
- Per-project task templates exist for templates; project-scoped *recurring*
