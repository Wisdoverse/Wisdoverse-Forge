...
| **CLI agent image**       | The local image a container agent runs, tagged `agentforge-agent:<tool>` (e.g. `agentforge-agent:codex`). The auto-updater pulls the matching registry repo `${AGENT_REGISTRY}/agent-<tool>:<tag>` and re-tags it to that local ref, so the next spawn resolves the current CLI. `claude` has no public image and is excluded.                      |
| **Image auto-update**     | Deployment-side poller (off by default, `CLI_IMAGE_AUTO_UPDATE_ENABLED`) that pulls newer `agent-<tool>` overlays on an interval so the next spawn runs the current CLI. Running agents are untouched. Status: `GET /api/v1/admin/cli-images` (admin-only).                                                                                         |
