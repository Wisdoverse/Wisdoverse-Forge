## Description

<!-- Provide a brief description of your changes. What problem does this solve? -->

## Type of Change

<!-- Check all that apply -->

- [ ] Bug fix (non-breaking change that fixes an issue)
- [ ] New feature (non-breaking change that adds functionality)
- [ ] Breaking change (fix or feature that would cause existing functionality to not work as expected)
- [ ] Refactoring (no functional changes)
- [ ] Documentation update
- [ ] Performance improvement
- [ ] Test addition or update
- [ ] CI/CD or infrastructure change

## Related Issues

<!-- Link any related issues using #issue_number -->

Fixes #
Related to #

## Changes Made

<!-- List the main changes made in this PR -->

-
-
-

## Screenshots / Demo

<!-- If applicable, add screenshots or a demo video -->

## Testing

<!-- Describe the tests that you ran to verify your changes -->

### Test Configuration

- Node version:
- OS:

### Tests Performed

- [ ] Unit tests pass (`npm test`)
- [ ] Integration tests pass
- [ ] E2E tests pass
- [ ] Manual testing performed

### Test Coverage

<!-- If applicable, describe the test coverage for your changes -->

## Checklist

### Code Quality

- [ ] My code follows the project's style guidelines
- [ ] I have performed a self-review of my code
- [ ] I have commented my code, particularly in hard-to-understand areas
- [ ] My changes generate no new warnings or errors
- [ ] I have added types for all new code (TypeScript)

### Documentation

- [ ] I have updated the documentation accordingly
- [ ] I have updated the CHANGELOG.md (if applicable)
- [ ] I have added JSDoc comments for public APIs

### Security

- [ ] I have considered security implications of my changes
- [ ] No sensitive data (API keys, passwords) is exposed
- [ ] Input validation is properly implemented (if applicable)

### Dependencies

- [ ] No new dependencies added
- [ ] New dependencies are justified and reviewed

## Deployment Notes

<!-- Any special considerations for deployment? Database migrations, environment variables, etc. -->

## Additional Notes

<!-- Any other information that reviewers should know -->

---

<!--
Reviewer Guidelines:
1. Check code quality and adherence to project standards
2. Verify tests are comprehensive
3. Review for security vulnerabilities
4. Ensure documentation is updated
5. Test locally if significant changes
-->
