...
describe('chatErrorMessage', () => {
  ...
  test('maps missing conversations to an agent chat navigation step', () => {
    ...
    expect(message).toBe(
      'Check conversation again to load the chat history. Open Agents, choose this agent again, then open Chat. This agent or conversation may have changed.'
    )
  ...
  test('maps server errors without exposing transport text', () => {
    ...
    expect(message).toBe(
      'Chat was not cleared. Wait a few minutes, then clear chat again if you still want to remove the messages. Forge could not update this chat right now. If it still fails, ask an owner or admin to check this agent chat.'
    )
  ...
  test('maps unusable conversation data without exposing raw response wording', () => {
    ...
    expect(message).toBe(
      'Check conversation again to load the chat history. Choose Check conversation again. Forge could not read this conversation.'
    )
  ...
  test('uses the visible retry action for unknown conversation load failures', () => {
    ...
    expect(message).toBe(
      'Check conversation again to load the chat history. Choose Check conversation again. If it still fails, ask an owner or admin to check this agent chat.'
    )
  ...
  test('uses the clear chat action for unknown clear failures', () => {
    ...
    expect(message).toBe(
      'Chat was not cleared. Clear chat again if you still want to remove the messages. If it still fails, ask an owner or admin to check this agent chat.'
    )
