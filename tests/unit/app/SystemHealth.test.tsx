...
describe('SystemHealth', () => {
  test('explains app health with user-facing labels', async () => {
    ...
    expect(screen.queryByText('Agent Work Starter')).toBeNull()
    expect(screen.queryByText('Starts file-work agents')).toBeNull()
    expect(screen.queryByText(/may stop reporting/i)).toBeNull()
  ...
  test('shows every check as ready when every area is working', () => {
    useAdminStore.setState({
      ...originalAdminState,
      health: {
        status: 'healthy',
    ...
    expect(screen.getByText(/project file area agents use when they edit files/i)).toBeDefined()
    expect(screen.queryByText(/prepared project files agents use for file work/i)).toBeNull()
  ...
  test('hides raw service error details from health check rows', async () => {
    ...
    expect(screen.getByText(/This area had a connection problem/i)).toBeDefined()
    expect(screen.queryByText(/This area reported a connection problem/i)).toBeNull()
  ...
  test('turns service setup errors into owner or admin next steps', async () => {
    ...
    expect(screen.queryByText(/This area reported/i)).toBeNull()
