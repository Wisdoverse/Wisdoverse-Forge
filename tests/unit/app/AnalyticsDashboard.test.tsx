...
describe('AnalyticsDashboard · ActivityBarChart', () => {
  ...
  test('guides an empty activity range toward running a first task', () => {
    ...
    expect(screen.getByText('Work steps appear after an agent finishes a task')).toBeDefined()
    ...
    expect(screen.queryByText('Tool use appears after an agent finishes a task')).toBeNull()
  ...
  test('points beginners at the busiest low-success tool first', () => {
    ...
    const nextStep = screen.getByTestId('analytics-next-step')
    expect(nextStep).toHaveTextContent('Check the busiest work step first')
    expect(nextStep).toHaveTextContent('Computer step completed cleanly only 42%')
    expect(nextStep).toHaveTextContent('check the recovery notes')
    expect(nextStep).not.toHaveTextContent('retry steps')
    ...
    expect(screen.getByText('Work steps')).toBeDefined()
    expect(screen.getByText('Work steps used most')).toBeDefined()
    expect(screen.getByText('Busiest work step')).toBeDefined()
    expect(screen.getAllByText('Computer step').length).toBeGreaterThan(0)
    expect(screen.queryByText('Tool steps')).toBeNull()
    expect(screen.queryByText('Tools used most')).toBeNull()
    expect(screen.queryByText('Busiest tool')).toBeNull()
