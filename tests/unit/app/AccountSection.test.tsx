...
describe('AccountSection', () => {
  ...
  test('restores a hidden Getting Started guide and confirms the result', async () => {
    ...
    expect(screen.getByText(/If the setup checklist is hidden/i)).toBeDefined()
    expect(screen.queryByText(/If Start is hidden/i)).toBeNull()
    expect(screen.getByText(/choose Show setup checklist/i)).toBeDefined()
    expect(screen.getByText(/add it back to the left menu/i)).toBeDefined()
    expect(screen.getByText(/New sign-ins still open Tasks by default/i)).toBeDefined()
