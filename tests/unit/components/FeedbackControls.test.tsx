...
describe('FeedbackControls', () => {
  test('explains context feedback choices in beginner language', () => {
    ...
    expect(screen.getByText('Was this context item helpful?')).toBeInTheDocument()
    expect(
      screen.getByText('Your answer helps future tasks choose safer, more useful context items.')
    ).toBeInTheDocument()
  ...
  test('shows recovery guidance when feedback cannot be saved', async () => {
    ...
    expect(alert.textContent).toContain('this context item')
