...
test.describe('opencode context injection E2E', () => {
  test.setTimeout(180_000)

  test('writes opencode AGENTS.md and adapter report from a runtime envelope', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'agentforge-opencode-context-e2e-'))
    ...
    try {
      ...
      const agentsMd = await readFile(path.join(home, '.config/opencode/AGENTS.md'), 'utf8')
      ...
      expect(agentsMd).toContain('<!-- agentforge-context:start v1 -->')
      expect(agentsMd).toContain('These instructions are generated for opencode')
      expect(agentsMd).toContain('~/.config/opencode/AGENTS.md')
      expect(agentsMd).toContain('Prod deploy rule')
      expect(agentsMd).toContain('Run make prod-ext after main pipeline succeeds.')
      expect(agentsMd).toContain('[redacted: secret_detected]')
      expect(agentsMd).not.toContain('raw-token-should-not-be-written')
      expect(agentsMd).toContain('prod-ext-check v1')
      expect(agentsMd).toContain('/home/agent/.config/opencode/skills/project/prod-ext-check')
      expect(agentsMd).toContain('Degradation: budget_truncated')
      expect(agentsMd).toContain('<!-- agentforge-context:end -->')
    ...
    skills_mount: [
      {
        name: 'prod-ext-check',
        version: 1,
        path: '/home/agent/.config/opencode/skills/project/prod-ext-check',
      },
    ],
