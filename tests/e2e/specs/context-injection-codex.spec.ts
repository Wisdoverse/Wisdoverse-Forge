...
test.describe('Codex context injection E2E', () => {
  test.setTimeout(180_000)

  test('writes Codex AGENTS.md and adapter report from a runtime envelope', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'agentforge-codex-context-e2e-'))
    ...
    try {
      ...
      expect(agentsMd).toContain('These instructions are generated for Codex')
      ...
      expect(agentsMd).toContain('/home/agent/.codex/skills/project/prod-ext-check')
    ...
    skills_mount: [
      {
        name: 'prod-ext-check',
        version: 1,
        path: '/home/agent/.codex/skills/project/prod-ext-check',
      },
    ],
